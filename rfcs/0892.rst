...
specified in the ``format()`` method and comparison methods. Specifies
how to round the value when the calculation is stopped midway due to the
...
``$maxExpansionScale`` and ``$roundMode``, which are not automatically
set from the calculation result, differ between objects during
calculation.
...
-  ``$roundMode`` has a higher priority in the following order:
...
``$roundMode`` would be ``PHP_ROUND_HALF_UP``.
