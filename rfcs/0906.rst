...
equality comparison (``==``). Given the following example: g
...
-  HTTP routers: They need the normalized and decoded representation of
   some HTTP components (i.e. path)
...
-  SAML login: Service providers should do the least amount of
   processing of the login URL of identity providers in order not to
   break the SAML signature, therefore they work with the raw encoded
   URIs
