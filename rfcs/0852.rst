...
It also proposes that when a GMP object is cast to bool, change it so
that ``0`` is ``false`` and everything else is ``true``, like an int.
...
#. Of the six comparison methods ``comp()``, ``eq()``, ``gt()``,
   ``gte()``, ``lt()``, ``lte()``, remove all except ``comp()``. Then
   rename the ``comp()`` method to ``compare()``.
...
In fact, only the ``comp()`` method is needed to fulfill the
functionality of the other five methods.

Adding methods later is easy, but removing them always involves a BC
Break. Providing 6 methods from the start is obviously overkill, so we
will only provide ``comp()``.
...
"comp". The expression "cmp" is mainly used for functions, and the
expression "compare" is used for class methods.

e.g. https://www.php.net/manual/ja/splpriorityqueue.compare.php
https://www.php.net/manual/ja/collator.compare.php

Therefore, follow these conventions and rename ``comp()`` to
``compare()``.
...
Based on the idea that we shouldn't provide an API that looks "rich"
from the get-go, I suggest removing all of these roundings.
...
the nature of GMP, such code is likely to be quite rare.
...
Question: Fix up BCMath Number Class
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Question: Change GMP bool cast behavior
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
