...
only deals with absolute URIs that have a scheme component (i.e.
``https``).
...
.. code:: php
   ...
   echo $url->getHumanFriendlyHost();               // example.com
...
``getHumanFriendlyHost()`` method comes handy to retrieve the host in a
...
which is most suitable for machine processing (host names using IDNA
characters are translated to ASCII characters), while the
``toHumanFriendlyString()`` method is a user-friendly representation
that displays the host as a Unicode string.
