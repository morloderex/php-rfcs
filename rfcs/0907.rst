...
RFC 3986 doesn't support IDNA (as it is reserved for RFC 3987), and it
only accepts Unicode character if they are properly encoded.
...
only deals with URIs having a scheme component (i.e. ``https``). WHATWG
supports IDNA, and it accepts Unicode characters in each component.
...
As opposed to RFC 3986, WHATWG automatically transforms the input URI
string during parsing (e.g. removes superfluous ``/`` characters after
the scheme).

Reference resolution
~~~~~~~~~~~~~~~~~~~~

Reference resolution is a process which turns a relative URI into an
absolute one by applying it to a base URI. I.e. resolving ``/foo`` on
``https://example.com/`` would result in ``https://example.com/foo``.
Both RFC 3986 and WHATWG support this concept.

Component recomposition
~~~~~~~~~~~~~~~~~~~~~~~

It is the process of recomposing the distinct URI components to an URI
...
Normalization and required transformations during parsing are especially
important when it comes to comparing URIs because they reduce the
likelihood of false positive results, as URI comparison is effectively
checking whether two URIs represent the same resource.
...
equal, then the 2 URIs are equal too. Usually, it's a good idea to
disregard the fragment component.
...
recomposing (converting the parsed structures back to strings) URIs
based on both RFC 3986 and the WHATWG URL specifications, as well as
resolving references (turning a (relative) URI to an absolute one by
applying a base URI to it). For this purpose, the following internal
...
successfully with some (soft) validation errors. When instantiating a
WHATWG ``Url`` via its constructor, only soft errors are passed by
reference, while hard errors are thrown. The ``Uri\WhatWg\Url::parse()``
method also returns soft error. The following example demonstrates these
cases:
...
.. code:: PHP
   ...
   // Soft errors when using the parse() method

   $url = Uri\WhatWg\Url::parse(" https://example.org", null, $errors);
   var_dump($url->toMachineFriendlyString());        // https://example.org
   var_dump($errors[0]->type);                       // enum(Uri\WhatWg\WhatWgErrorType::InvalidUrlUnit)

   // Soft errors when using the constructor
...
getter-based approach because each URI component actually needs multiple
getters in order to best serve the vastly different needs users may
have. In order to best understand why this is necessary

This topic is discussed in details
`below <https://wiki.php.net/rfc/url_parsing_api#how_special_characters_are_handled>`__.
For now, let's see a very basic example for accessing URI components:
...
the usage of quite some transformations (i.e. removal of extraneous
``/`` characters in the scheme, lowercasing some URI components,
application of IDNA encoding). On the other hand, RFC 3986 leaves the
originally supplied URI intact by default.

It may seem peculiar, but ``Uri\WhatWg\Url`` has two "toString" methods
in order to properly and safely support IDNA (internationalized domain
names):

.. code:: php

   $url = new Uri\WhatWg\Url("HTTPS://////EXAMPLE.com");
   echo $url->toMachineFriendlyString();             // https://example.com/

   $url = new Uri\WhatWg\Url("HTTPS://////你好你好.com");
   echo $url->toMachineFriendlyString();             // https://xn--6qqa088eba/
   echo $url->toHumanFriendlyString();               // https://你好你好/

The ``toMachineFriendlyString()`` method recomposes the URI in a format
which is most suitable for machine processing (domains using IDNA
characters are translated to ASCII characters), while the
``toHumanFriendlyString()`` method is a much more user-friendly
representation that displays the host as a Unicode string. Besides just
user-friendliness, this is important for security reasons, alleviating
the human risk factor in exploits. E.g. "xn--google.com" could deceive
the uninitiated reader that it is a Google domain, however the IDNA
domain decodes to "䕮䕵䕶䕱.com” in fact.

As RFC 3986 doesn't support IDNA (only `RFC 3987
does <https://datatracker.ietf.org/doc/html/rfc3987>`__), it's not a
concern for ``Uri\Rfc3986\Uri``. Therefore, it misses the above methods,
although it has a "toString()" and a "toNormalizedString()" method:
...
.. code:: php
   ...
   $uri = new Uri\Rfc3986\Uri("HTTPS://你好你好.com");
   var_dump($uri);                                    // NULL
...
.. code:: php
   ...
   echo $url->resolve("/foo")->toMachineFriendlyString();  //  https://example.com/foo
