...
.. code:: php
   ...
   $url = Uri\WhatWg\Url::parse("invalid uri", $errors); // null is returned, and an array of WhatWgError objects are passed by reference to $errors

As it can be seen in the example above, 'Uri\WhatWg\Url::parse()'
returns additional information about the triggered validation errors,
`as specified by
WHATWG <https://url.spec.whatwg.org/#validation-error>`__. But not all
validation errors result in a parsing failure. These are called
"soft-errors".
