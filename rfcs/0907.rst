...
WHATWG URI via its constructor, only soft errors are passed by
reference, while hard errors are thrown. The following example
...
have. This topic is discussed in details
...
originally supplied URI intact by default. For reference, the following
algorithms are implemented:

-  RFC 3986: https://datatracker.ietf.org/doc/html/rfc3986#section-5.3
-  WHATWG: https://url.spec.whatwg.org/#url-serializing

Peculiarly, ``Uri\WhatWg\Url`` has two "toString" methods in order to
properly and safely support IDNA (internationalized domain names):
...
but it has a regular "toString" method:
...
section, the two URIs should be indeed equal, as a result of
...
which is an optional process for canonizing different URI identifying
the same resource to the same URI. E.g. the ``https://EXAMPLE.com`` and
the ``HTTPS://example.com/`` URIs both refer to the same resource, so
they can be normalized to ``https://example.com``. WHATWG doesn't have
this concept (all transformations are applied during parsing), therefore
neither ``Uri\WhatWg\Url`` supports any such capabilities. However,
``Uri\Rfc3986\Uri`` supports normalization the following way:
...
resources. Therefore, the ``equals()`` method can be used for comparing
URIs. First, the method only accepts URI objects of the same
specification, since it doesn't make sense to compare URIs of different
standards. Then they normalize (if applicable) and recompose the URI
