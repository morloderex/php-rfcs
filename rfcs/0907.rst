...
absolute one by applying it to a base URI: resolving "``/foo``" on
``https://example.com/`` results in ``https://example.com/foo``. Both
RFC 3986 and WHATWG support this concept.
...
.. code:: PHP
   ...
   echo $url->toMachineFriendlyString();             // https://example.org
...
.. code:: php
   ...
   echo $url->toMachineFriendlyString();                         // https://example.com/foo

   $url = Uri\WhatWg\Url::parse("https://test.com/foo", "https://example.com");
   echo $url->toMachineFriendlyString();                         // https://test.com/foo
...
getter-based approach because each URI component actually has be
...
In case of ``Uri\WhatWg\Url``, the host field can also be retrieved in a
human-friendly way. This is needed to safely support IDNA
...
.. code:: php
   ...
   echo $url->getHumanFriendlyHost();                 // 🐘.com
...
due to the same reason as it has two getters for the host component.
...
.. code:: php
   ...
   $uri = new Uri\Rfc3986\Uri("https://%e4%bd%a0%e5%a5%bd%e4%bd%a0%e5%a5%bd.com"); // percent-encoded form of https://你好你好.com
...
.. code:: php
   ...
   $uri = new Uri\Rfc3986\Uri("https://example.com#foo");
   $uri->equals(new Uri\Rfc3986\Uri("https://example.com")));   // false
   ...
   $url->equals(new Uri\WhatWg\Url("https://example.com")));   // false
...
Fortunately, this overhead is minimal for complex URLs thanks to the
huge efforts of the maintainers of the Lexbor and the uriparser
libraries.
...
-  ``parse_url()``: ``0.000813``
-  ``Uri\Rfc3986\Uri``: ``0.000902 sec``
-  ``Uri\WhatWg\Url``: ``0.000813 sec``
