...
.. code:: php
   ...
   $foo is interable;  // Equivalent of is_iterable().

The constants ``true``, ``false``, and ``null`` are now fully defined
types in PHP, so can also be matched as a type.
...
properties of that object. If an object pattern is used, union and
intersections are not allowed at the top level. (However, they can be
used in sub-patterns on individual properties. See below.) The
properties must be accessible in the scope in which the pattern
executes. That is, a pattern evaluated outside the class may only match
against public properties; a pattern inside the class may match against
public, private, or protected; a pattern in a child class may match
against protected properties of its parent but not private; etc.
...
that is, you cannot bind to a property of an object, and cannot bind to
a variable-variable.
