DOMDocument HTML5 Parser
========================

:PHP-RFC: 0845
:Title: DOMDocument HTML5 Parser
...
become the de facto standard for websites over the past 10 years.
...
valid tags. Then there's also problems with certain elements nestings
not allowed in HTML4, but allowed in HTML5, causing incorrect document
...
it seems like this won't happen anytime soon. This RFC proposes
leveraging an alternative HTML5 parser to create the libxml2 document
tree. This will introduce two new methods:
``DOMDocument::loadHTML5(string $html, int $options=0)`` and
``DOMDocument::loadHTML5File(string $path, int $options=0)``. This
parser seamlessly integrates with the DOM extension, ensuring
compatibility for all existing code and third-party extensions.

Proposal
--------

The most important requirement is that the HTML5 parser must integrate
seamlessly with the DOM extension. This means that using the HTML5
parser must be a simple drop-in replacement. You will still be able to
use all the existing APIs to manipulate and traverse DOM documents and
nodes.

We propose the introduction of two new methods
``DOMDocument::loadHTML5(string $html, int $options=0)`` and
``DOMDocument::loadHTML5File(string $path, int $options=0)`` instead of
replacing the existing ones. This ensures full backwards compatibility
for applications that work with legacy HTML4 documents, and want the
HTML4 behaviour.

Serializing the document back to a string or to a file will be possible
with the existing ``DOMDocument::saveHTML`` and
``DOMDocument::saveHTMLFile`` methods.

The options argument
~~~~~~~~~~~~~~~~~~~~

Just like the existing load methods, these two new methods also take an
optional options argument. The options for the load methods change the
way the parser behaves. The only two options that will have an effect
for the new methods are ``LIBXML_HTML_NOIMPLIED`` and
...
interopability (e.g. with simplexml and xsl). Third-party extensions
...
-  Gumbo: https:*github.com/google/gumbo-parser.
   ...
   alternative character encoding support is problematic: it currently
   only supports UTF-8 documents. Support for other text encodings is
   lacking, necessitating a prior conversion. Moreover, reparsing logic
   for character encoding meta tags is absent.
...
the parsing rules. Browsers, and the proposed loadHTML5 methods, *only*
perform the parsing rules checks. An example of something that's fine
for a HTML5 parser, but not fine for a conformance checker is the
following document:
...
When parsing a document, potential parse errors may occur. With the
existing load methods of DOMDocument, a parser error results in an
E_WARNING by default. *However*, you can use
``libxml_use_internal_errors(true)`` to store the errors inside an
array. In this case, no warning will be generated and the parse errors
may be inspected using ``libxml_get_errors()`` and
...
DOMDocument / SimpleXML / ...". To offer a seamless HTML5 parser
replacement, my current implementation follows the same error handling
as described above. That means, by default we will emit an E_WARNING. If
...
affect loadHTML5File in the same way as it does loadHTMLFile. The
advantage would be consistency, but I don't know if this is what the
community wants. I'm leaving this for a secondary vote for the community
to decide on.
...
particular, how does the performance of ``DOMDocument::loadHTML5``
...
were removed. I created a PHP script that invokes each parser 1000
times. I ran the experiment on an i7-4790 with 16GiB RAM.

Results
^^^^^^^

The following graph shows the results. The blue bar shows the parse time
in seconds for loadHTML, and the orange bar does so for loadHTML5. Lower
is better. The black vertical line indicates the minimum & maximum
measured times for each bar. First of all, some measurements on the far
left are very low. That's because those sites primarily generate their
content using JavaScript. Hence, there are not many HTML nodes in the
document. Some sites also show a geo-blocked page, so these pages are
rather simple and will be parsed quickly. Second, we can see that
loadHTML5 is consistently faster than the existing loadHTML method,
despite having to do a conversion.
...
extension. The Lexbor library is fairly big. The reality is however that
a lot of the library is not actually used. Diving into each source file
and ripping out functions that are not used is time-consuming and
difficult however. Furthermore, this would make syncing upstream changes
also more difficult.
...
#. Interopability problems with other extensions (both within php-src
   and third-party).
...
#. Whether ``DOMDocument::loadHTML5(string $html, int $options=0)`` and
   ``DOMDocument::loadHTML5File(string $path, int $options=0)`` should
   be introduced. This requires 2/3 majority.
#. Whether loadHTML5File should respect the resolver set by
   ``libxml_set_external_entity_loader``. This requires 50% majority.
