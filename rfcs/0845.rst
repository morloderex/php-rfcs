...
.. code:: php
   ...
   namespace DOM {
       ...
       class XMLDocument extends Document {
           ...
           public static function fromFile(string $path, int $options = 0);
           public static function fromString(string $source, int $options = 0);
       }
...
This RFC adds three new classes, and aliases, but the existing
DOMDocument class as-is. ``DOMNode::ownerDocument`` gets its type
changed from ``?DOMDocument`` to ``?DOM\Document``. As PHP's type checks
happen at runtime instead of statically, this shouldn't affect
assignments. Overriding a property in a child class of DOMNode would
cause a compile error. However, overriding properties is useless in PHP
anyway, so this is only a minor break. Therefore, this feature is almost
purely opt-in.
