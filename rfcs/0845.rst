...
loadXML methods, but the behaviour doesn't change.
...
``LIBXML_NOERROR``. Here's an overview of the other options that are
unimplemented and the reason why:
...
|                          | because it requires some hacks. It can   |
...
future *may* allow a complete transition to the namespaced variants.
This means for example that there will be an alias ``DOM\Document`` for
...
There is one primary vote, and there are 2 secondary votes:
