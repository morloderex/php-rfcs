...
-  The content type is needed because ``request_parse_body()`` also
...
Like the automatically invoked RFC1867 parsing, ``request_parse_body()``
will fail under various conditions.
...
RFC1867 parsing. However, with a dedicated method we can benefit from
exceptions to make parsing errors unmissable. A new Exception class
...
throws. This is because the function receives input from the client that
cannot be received again. It is thus not safe to call
``request_parse_body()`` twice for the same request without providing an
input stream.

Furthermore, calling ``request_parse_body()`` with an unsupported
Content-Type also throws the same ``RequestParseBodyException``.
