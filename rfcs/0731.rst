...
-  Move the optimizer into core (e.g. move code related to opcode
   optimizations from ``ext/opcache`` to a new folder ``ext/optimizer``
   loaded before opcache). Keep all of the functionality related to
   caching in the zend_extension Zend Opcache (some build environments
   may not support or have a use case for any forms of shared memory
   caching).
