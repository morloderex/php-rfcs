...
possible to import "old DOM nodes" into the "new DOM" and vice versa.
This should also help library developers migrate: they could write an
adapter layer or adapter helper functions such that the "new DOM"
classes are imported into the "old DOM" that the library may use. This
would, for quite a few libraries, avoid the requirement to maintain two
different versions of the same library. Of course there are also
libraries that expose the DOM classes that are used, they would have a
harder time pulling off an adapter interface and there might not be an
easy solution for this except to place the burden on the user of such a
library.
...
existing importNode methods' arguments.

These methods can fail if an unsupported node is imported (e.g. a
document node itself). The methods in DOMDocument can return false on
