...
``Deque`` and ``Vector`` based on php-src's data structure
implementations (mostly ``SplFixedArray`` and ``ArrayObject``) instead
(and based on my past PECL/RFC experience, e.g. with
``runkit7``/``igbinary``)
