...
Besides accessors, the ``Uri\Uri`` contains a ``toString()`` method too.
...
``__toString()`` method, because of the ``$uri`` object being converted
to its string representation (``https://example.com``) which is then
matched against ``HTTPS://example.com``. However, as we will see in the
next paragraphs, the two URIs should be indeed equal, as the result of
...
Additionally, the RFC 3986 has the notion of
...
be also normalized, or exact string match is performed? Should the
segment be excluded from the comparison? Would the URI be parsed based
on the same standard as the callee object? These are the questions which
don't have to be answered when only the ``Uri\Uri`` parameter type is
supported.

These pressing questions - combined with the fact that the construct is
not supported in userland - led not to overload the equality operator.
