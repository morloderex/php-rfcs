...
resulting int if possible by default. In case of integer under-/overflow
the value will be casted to a float (double) and rounded based on the
floating point number as it's done currently.
...
``force_float=true`` to get the new behavior by default but the previous
behavior can be forced with ``force_float=true``.
