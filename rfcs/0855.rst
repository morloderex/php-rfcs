...
This RFC proposes to perform rounding on given int and return the
resulting int if possible. In case of integer under-/overflow the value
will be casted to a float (double) and rounded based on the floating
...
::
   ...
   ceil(int): int
   floor(int): int
   round(int, precision: >= 0): int
   round(int, precision: < 0): int|float // implicit cast to float only in case of integer under-/overflow
...
8.4 behavior (deprecation phase):

::

   Deprecated: Rounding an int will return an int instead of a float in the future, consider casting argument 'num' to float to keep current behavior.
   float(1)
   float(1)
   Deprecated: Rounding an int will return an int instead of a float in the future, consider casting argument 'num' to float to keep current behavior.
   float(9.87654321098766E+17)
...
report an int. Because of that there will be a deprecation phase so
...
``ceil`` and ``floor`` would return an int. For as long as the
deprecation phase the resulting int gets casted back to ``float`` before
return.
