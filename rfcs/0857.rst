...
A future scope of the RFC could be to remove the previously possible way
of checking not instance of. So instead of
``!$object instanceof MyClass`` being equal to
``$object !instanceof MyClass`` it would now be interpreted as the
following code ``(!$object) instanceof MyClass``
...
:Slug: is_not_instanceof
:Wiki URL: https://wiki.php.net/rfc/is_not_instanceof
