...
[I am strongly considering changing this for this version and allowing
just set, as that would allow "private(set) string $foo" for the
super-common case of public-read, private-write. I really think it's a
common enough case that we should do that.]
...
specifying asymmetric visibility is not permitted. (Hooks, of course,
are.)
...
The ``readonly`` flag is, on its own, two flags in one: write-once and
``private(set)``. While both have their use cases, there are ample times
where only one or the other is desired. For instance, the following code
from Crell/Serde (slightly simplified for this example) wants to use
``readonly``, but because of the implied ``private(set)`` it causes
issues:
