...
Specifying a parameter name but no type is equivalent to specifying a
type of ``mixed``, just as it is on methods. Whether that wide of a type
is appropriate or not is up to the developer to decide.
...
-  ``get_mangeled_object_vars()``: Use raw value
...
``get_mangeled_object_vars()`` was intended as a long-term replacement
...
There is a new global enum, ``HookType``:
...
-  ``getHook(HookType $hook): ?\ReflectionMethod`` returns the
