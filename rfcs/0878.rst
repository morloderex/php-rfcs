...
it makes ``$this->[propertyName]`` will refer to the "unfiltered" value
of the property. When accessed from anywhere else,
``$this->[propertyName]`` calls will go through the relevant hook. This
is true for all hooks on the same property. This includes, for example,
writing to a property from the ``get`` hook; that will write to the
backing property, bypassing the ``set`` hook.
...
trigger an error. If it just uses ``get``, then obtaining a reference is
not allowed as it would allow any arbitrary code to modify a
possibly-private property. If the property does use ``&get``, it implies
the class author is aware of that exposure situation and sees it as a
feature, not a bug.
...
the issue of bypassing a write isn't relevant. Additionally, it is
unlikely that the ``get`` operation will return a reference to a
property of the object to begin with. For that reason, ``&get`` by
reference is allowed for virtual properties, regardless of whether or
not there is a ``set`` hook.
...
That would, as above, bypass any ``set`` hooks that have been defined.
...
superior API choice. Therefore, while a ``set`` hook may be applied to
array properties attempting to write to a key of a backed array property
that has hooks (using ``[]`` or ``['some key']``) will result in a
runtime error being thrown.
...
individual key is safe. Additionally, ``&get`` hooks are allowed for
arrays as well, provided there is no ``set`` hook, and writing to a key
on an array returned by reference is also allowed.

It is a compile error for an array property to have both ``&get`` and
``set`` hooks.
...
.. code:: php
   ...
   class C
   {
       ...
       public array $get {
           get {
             if (empty($this->get)) $this->get['first'] = 1;
             return $this->get;
           }
       }
       ...
       public array $getRef {
           &get {
             if (empty($this->getRef)) $this->getRef['first'] = 1;
             return $this->getRef;
           }
       }

       public array $hasSet {
           get => $this->hasSet + ['extra' => 'stuff'];
           set {
   ...
   // OK, returns the whole array, lazy-initialized.
   print_r($c->get);

   // OK; the hook returns the whole array,
   // then the 'beep' key is accessed.
   print $c->get['first'];

   // OK.  The entire array is overwritten 
   // like any other public property.
   $c->get = ['new' => 'array'];

   // Errors, because you cannot modify 
   // the array when it's returned by value.
   $c->get['beep'] = 'boop';
   ...
   // OK; the hook returns the array by reference,
   // then the 'beep' key is accessed.
   print $c->getRef['first'];

   // OK.  The entire array is overwritten 
   // like any other public property.
   $c->getRef = ['new' => 'array'];

   // OK, because the write here happens to the
   // array reference returned by the &get method,
   // which then propagates back to the property.
   $c->getRef['beep'] = 'boop';
...
which disables writing directly to the ``$elements`` property. Returning
by reference would also be allowed, if desired.
