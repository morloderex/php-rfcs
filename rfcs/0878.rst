...
hook, the "unfiltered" value of the property is available as
``$this->propertyName``. Outside of the hook, ``$this->propertyName``
calls will still go through the relevant hook.
...
not on dynamic forms of it like ``$prop = 'beep'; $this->$prop``. That
will not trigger a backing value.
...
hook and it does not use ``$this->fulName``. Because it is virtual,
