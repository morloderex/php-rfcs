...
An interface may not specify if the ``get`` hook should return by
reference. A class may satisfy a ``get`` requirement with either a
by-val or by-ref ``get`` method, as appropriate.
