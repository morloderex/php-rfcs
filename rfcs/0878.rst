...
#. We may create an implicit copy to the array, and silently pass it
   ...
   figuring out \_what\_ in the array has changed, or rechecking it in
   full.
...
With asymmetric visibility that was previously proposed, the example can
...
previously proposed `Asymmetric
Visibility </rfc/asymmetric-visibility>`__ RFC would provide a much more
elegant approach in that case:
...
+-------------+-------------+-------------+-------------+-------------+
| Backed      | `           | Allowed     | Allowed     | Allowed     |
|             | `&get``\ \* |             |             |             |
+-------------+-------------+-------------+-------------+-------------+
...
\* \`&get`-only allows for some use cases of arrays to have a get hook
and still behave "normally" as far as index writing goes. For example, a
lazy-initialized array that is then "fully public" thereafter:
...
The array-offset rules cannot be enforced at compile time, as we cannot
reliably tell at compile time what the type and hooks of the property
will be. (It may defined in a different file.)
